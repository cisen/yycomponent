...
function getModuleData(props) {
  ...
  const moduleName = /^\/?components/.test(pathname) ?
          ...
          'yycomponent' : pathname.split('/').filter(item => item).slice(0, 2).join('/');
  // 这里组装左边栏的数据
  let moduleData;
  if (moduleData = moduleName === 'components' || moduleName === 'docs/react' ||
          moduleName === 'changelog' || moduleName === 'changelog-cn')
  {
    moduleData = [...props.picked.components, ...props.picked['docs/react'], ...props.picked.changelog]
  } else if (moduleName === 'yycomponent' || moduleName === 'docs/yycomponent') {
    moduleData = [...props.picked.yycomponent, ...props.picked['docs/yycomponent'], ...props.picked.changelog]
  } else {
    moduleData = props.picked[moduleName]
  }
  // const moduleData = moduleName === 'components' || moduleName === 'yycomponent' || moduleName === 'docs/react' ||
  //         moduleName === 'changelog' || moduleName === 'changelog-cn' || moduleName === 'docs/yycomponent' ?
  //         [...props.picked.components, ...props.picked.yycomponent, ...props.picked['docs/react'], ...props.picked.changelog] :
...
export default class MainContent extends React.Component {
  ...
  generateMenuItem(isTop, item) {
    ...
    const child = !item.link ? (
      <Link
        to={utils.getLocalizedPathname(/^components/.test(url) ?
          `${url}/` : /^yycomponent/.test(url) ?
          `${url}/`: url,
          locale === 'zh-CN')}
        disabled={disabled}
      >
