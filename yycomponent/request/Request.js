...
import {notification, Modal} from 'yycomponent';
...
const msgNode = (msg) =>{
  return (
    <div dangerouslySetInnerHTML={{__html: msg}}></div> //eslint-disable-line
  );
};
...
var processResponse = function (promise, url, sucCode, config) {
  ...
  return setPromiseTimeout(promise, config.options.timeout).then(function (res) {
    ...
    if (res.ok) {
      ...
      //后台出现错误,直接报错。
      notification.error({message: '', description: "网络错误", duration: 3, key: 'RequestError'});
      //Tip.danger({children: "服务器忙,请稍后重试"});
      return Promise.reject('网络错误:' + res.status + res.statusText, res);
    }
  }).then((rsp) => {
    // console.log(config);
    // debugger
    ...
    //处理状态码
    if (rsp.code === 302) {
      //防止多次弹出登录超时，设置一个变量来控制弹窗的输出
      !isTimeout && Modal.info({
        title: "提示",
        content: "登录超时，请重新登录",
        onOk: ()=> {
          isTimeout = false;
      ...
      notification.error({message: '', description: rsp.message, key: 'RequestError'}); //这个可能其它地方有用到，就没有动
      return rsp;
    } else {
      // notification.error({message: '', description: rsp.message, duration: 3});
      notification.error({message: '', description: msgNode(rsp.message), duration: 3, key: 'RequestError'});
      return Promise.reject(rsp.message || '未知错误');
      //Tip.danger({children: rsp.message});
    }
  }).then((json) => {
    if (!json) {
      return false;
    ...
    if (toString.call(reason) === '[object Promise]') {
      ...
      notification.error({message: '', description: '请求超时', key: 'RequestError'});
